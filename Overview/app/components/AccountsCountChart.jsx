function AccountsCountChart() {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-5 pt-5 pb-4">
      ...
      {/* Chart */}
      <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} style={{ width: '100%', height: 'auto' }} aria-label="Account's Count bar chart">
        {/* Y-axis ticks */}
        {yTicks.map((tick) => {
          const y = getY(tick);
          return (
            <g key={tick}>
              <text x={paddingLeft - 8} y={y + 4} textAnchor="end" fontSize="11" fill="#9ca3af">{tick}</text>
            </g>
          );
        })}
          ...
          return (
            <g key={d.name}>
              <path
                ...
                fill={d.color}
              />
              <text x={slotCenterX} y={chartHeight - 8} textAnchor="middle" fontSize="12" fontWeight="600" fill="#374151">
                {d.name}
              </text>
            </g>
          );
        ...
        {/* Baseline */}
        <line x1={paddingLeft} y1={getY(0)} x2={chartWidth - paddingRight} y2={getY(0)} stroke="#d1d5db" strokeWidth="1" />
      </svg>
    </div>
  );
}
