...
function BureauScoreLineChart() {
  ...
  const minTime = allPoints.length ? Math.min(...allPoints.map((point) => point.timestamp || 0)) : 0;
  const maxTime = allPoints.length ? Math.max(...allPoints.map((point) => point.timestamp || 0)) : 0;
  ...
  function xFor(timestamp, index, count) {
    if (minTime && maxTime && minTime !== maxTime) {
      return SCORE_PL + ((timestamp - minTime) / timeRange) * SCORE_IW;
    }
    if (count <= 1) return SCORE_PL + SCORE_IW / 2;
    return SCORE_PL + (index / (count - 1)) * SCORE_IW;
  }
  ...
  const xLabels = allPoints
    ...
    .filter((point, index, arr) => {
      if (!point.timestamp) return index === 0;
      const label = formatScoreDate(point.timestamp);
      return index === 0 || label !== formatScoreDate(arr[index - 1].timestamp);
    })
    .slice(0, 5);
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-5 pt-5 pb-4 h-full flex flex-col">
      ...
      >
        ...
        {xLabels.map((point, index) => {
          const x = xFor(point.timestamp, index, Math.max(xLabels.length, 1));
          return (
            <text key={`${point.timestamp}-${index}`} x={x} y={SCORE_CH - 33} textAnchor="middle" fontSize="11" fill="#111827">
              {formatScoreDate(point.timestamp)}
            </text>
          );
        })}
        ...
        {SCORE_BUREAUS.map((bureau) => {
          ...
          const points = rawPoints.map((point, index) => ({
            ...point,
            x: xFor(point.timestamp, index, rawPoints.length),
            y: yFor(point.score),
          }));
          ...
          return (
            <g key={bureau}>
              ...
              {points.map((point, index) => {
                const prevX = points[index - 1] ? points[index - 1].x : SCORE_PL;
                const nextX = points[index + 1] ? points[index + 1].x : SCORE_CW - SCORE_PR;
                const hitX = index === 0 ? SCORE_PL : (prevX + point.x) / 2;
                const hitW = index === points.length - 1
                  ? SCORE_CW - SCORE_PR - hitX
                  : (point.x + nextX) / 2 - hitX;
                return (
                <g
                  key={`${bureau}-${point.timestamp}-${index}`}
                  onMouseEnter={() => setHovered({ ...point, bureau })}
                  style={{ cursor: 'pointer' }}
                >
                  <rect x={hitX} y={SCORE_PT} width={hitW} height={SCORE_IH} fill="transparent" />
